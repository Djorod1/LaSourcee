-- =====================================================================
-- LaSourcee — Schéma SQLite (équivalent fonctionnel de schema.sql + migration_v2.sql)
...
CREATE TABLE utilisateur (
    ...
    langues         TEXT,
    profil_pro      TEXT,
    -- Parcours en trois champs distincts. Un champ libre unique melait
    -- le diplome, la filiere et l'ecole, et ne se recherchait pas.
    niveau_etudes   TEXT,
    domaine         TEXT,
...
CREATE TABLE question (
    ...
    maj_le       TEXT    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Mesures d'usage. Le nombre de reponses et le delai avant la
    -- premiere se recalculent par jointure, mais une jointure par
    -- ligne coute cher des que le fil s'allonge, et ces valeurs
    -- servent aussi de variables d'analyse.
...
CREATE TABLE verification_email (
    id_jeton        TEXT    PRIMARY KEY,
    -- Code court saisi a la main. Un lien peut echouer, etre coupe par
    -- la messagerie ou pointer ailleurs ; six chiffres se recopient
...
CREATE TABLE identite_externe (
    id_externe      INTEGER PRIMARY KEY AUTOINCREMENT,
    id_utilisateur  INTEGER NOT NULL,
...
--
-- L'horodatage est un entier epoch : la comparaison de fenetre devient
-- identique sur les trois moteurs, sans arithmetique de dates.

CREATE TABLE tentative_auth (
    id_tentative INTEGER PRIMARY KEY AUTOINCREMENT,
    cle          TEXT    NOT NULL,   -- « action|identifiant|adresse IP »
